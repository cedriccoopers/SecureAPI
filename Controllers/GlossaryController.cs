...
namespace SecureAPI.Controllers
{
    ...
    public class GlossaryController : ControllerBase
    {
        public static List<GlossaryItem> Glossary = new List<GlossaryItem>
        {
            new GlossaryItem
            {
                ...
                Definition = "A credential that can be used by an application to access an API."
                                + "It informs the API that the bearer of the token has been authorised to access"
                                + "the API and perform specific actions specified by the scope that has been granted."
            ...
            {
                 Term = "OpenID",
                 Definition = "An open standard for authentication that allows applications to verify users are who they say"
                            + "they are  without needing to collect, store, and therefore become liable for a user's login information"
            }
        };
        ...
        {
            var existingGlossaryItem = Glossary.Find(
                item => item.Term.Equals(glossaryItem.Term, StringComparison.InvariantCultureIgnoreCase));
